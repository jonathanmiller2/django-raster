...
from raster.models import Legend, RasterProduct, RasterLayer, RasterLayerBandMetadata, RasterLayerMetadata
...
class RasterView(View):
    ...
    def get_layer(self):
        ...
        if 'layer' in self.kwargs:
            data = self.kwargs.get('layer') #TODO: This probably needs to be adjusted for Product-Year-Day rather than layer name
...
class AlgebraView(RasterView):
    ...
    def get_ids(self):
        ...
        if 'product' in self.kwargs and 'year' in self.kwargs and 'day' in self.kwargs:
            p = self.kwargs.get('product')
            y = self.kwargs.get('year')
            d = self.kwargs.get('day')
            product_query = Q(name__icontains=p)
            product = get_object_or_404(RasterProduct, product_query)
